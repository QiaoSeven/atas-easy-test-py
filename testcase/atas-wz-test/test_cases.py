...
class TestCases:
    ...
    def test_create_case(driver, data, capture, logger, atas_api):
    ...
    @pytest.mark.parametrize("driver", browser_data, indirect=True)
    def test_delete_case(driver, data, capture, logger, atas_api):
        def_name = sys._getframe().f_code.co_name
        # 数据
        data = get_data[def_name]
        nv = NavigateTask(driver, data)
    ...
    @pytest.mark.parametrize("driver", browser_data, indirect=True)
    def test_create_filter(driver, data, capture, logger, atas_api):
        def_name = sys._getframe().f_code.co_name
        # 数据
        data = get_data[def_name]
        nv = NavigateTask(driver, data)
    ...
    @pytest.mark.parametrize("driver", browser_data, indirect=True)
    def test_edit_filter(driver, data, capture, logger, atas_api):
        def_name = sys._getframe().f_code.co_name
        # 数据
        data = get_data[def_name]
        nv = NavigateTask(driver, data)
        ...
        # which_column可选择"Single"/"All"/"None"。表示想在filter的Results展示多少列
        # if_verify可选择True/False。表示是否需要校验操作结果
        case.edit_filter(False, True)
        # if_project可选择True/False。表示是否需要从project菜单进入子层级
        # if_verify可选择True/False。表示是否需要校验操作结果
        driver.get_driver().quit()

    @pytest.mark.parametrize("driver", browser_data, indirect=True)
    def test_delete_filter(driver, data, capture, logger, atas_api):
        def_name = sys._getframe().f_code.co_name
        # 数据
        data = get_data[def_name]
        nv = NavigateTask(driver, data)
